...
import { getErpUserId, clearErpUserId } from '../../erp-user-id';
...
export default class RestClient implements Client {
  ...
  private checkResponseForSessionExpired = <T>(error: {
    ...
    // ERP iframe 연동 모드에서는 세션/JWT 만료 개념이 없다.
    // userId 누락 등으로 401/403이 발생할 수 있으므로, 여기서 강제 다이얼로그를 띄우지 않는다.
  ...
  constructor(baseUrl: string) {
    ...
    this.axios.interceptors.request.use((config) => {
      if (config.headers) {
        // ERP iframe integration: userId 기반(무상태) 인증
        const erpUserId = getErpUserId();
        if (erpUserId) {
          config.headers['X-User-Id'] = erpUserId;
        }
  ...
  login(model: JwtAuth): Promise<void> {
    // ERP iframe 연동 모드에서는 로그인 API를 사용하지 않는다.
    // (부모 프레임이 전달한 userId를 RestClient가 X-User-Id 헤더로 전달)
    return Promise.resolve();
  }
   ...
   * @param userId - 쿼리 파라미터로 전달된 userId (선택적)
   */
  loginFromSession(userId?: string): Promise<void> {
    // 기존 ERP 세션 기반 자동 로그인(JWT 발급) 흐름을 제거.
    // 필요 시 호출한 쪽에서 userId를 URL에서 캡처하여 sessionStorage에 저장하면 됨.
    // (userId 파라미터는 capture 단계에서만 사용)
    return Promise.resolve();
  }
  ...
  searchUsersForCollaboration(mapId: number, searchTerm: string, limit: number = 10): Promise<UserSearchResult[]> {
    const handler = (
      success: (users: UserSearchResult[]) => void,
      reject: (error: ErrorInfo) => void,
    ) => {
      const normalizedSearch = (searchTerm || '').trim();

      this.axios
        .get(`${this.baseUrl}/api/restful/maps/${mapId}/users/search`, {
          params: {
            q: normalizedSearch,
            limit: limit,
          },
        ...
        .then((response) => {
          const users: UserSearchResult[] = (response.data as any[]).map((u) => ({
            email: u.email,
  ...
  private parseResponseOnError = (response: any): ErrorInfo => {
    ...
    if (response) {
      ...
      if (result.isAuth === undefined) {
        result.isAuth = normalizedStatus === 403;
      }
    }
