...
class MockClient implements Client {
  ...
  loginFromSession(userId?: string): Promise<void> {
    // Mock implementation - always fails (no ERP session in mock)
    return Promise.reject({ msg: 'No ERP session found' });
  }
  ...
  searchUsersForCollaboration(mapId: number, searchTerm: string, limit: number = 10): Promise<UserSearchResult[]> {
    ...
    // 이미 협업 중인 사용자 이메일 수집
    const existingPerms = this.permissionsByMap.get(mapId) || [];
    const collaboratingEmails = new Set(existingPerms.map((p) => p.email.toLowerCase()));

    // 검색어로 필터링 (대소문자 무시)
    const searchLower = searchTerm.toLowerCase();
    const filtered = mockUsers
      .filter((user) => {
        const isCollaborating = collaboratingEmails.has(user.email.toLowerCase());
        if (isCollaborating) return false;
        return (
