...
const buildPersistenceManagerForEditor = (
  ...
  if (AppConfig.isRestClient()) {
    ...
    const result = new RESTPersistenceManager({
  ...
  return new MockPersistenceManager(exampleMap);
};
...
const EditorPage = ({ mapId, pageMode, zoom, hid }: EditorPropsType): React.ReactElement => {
  ...
  // ERP iframe 연동: userId는 CommonPage/loader 단계에서 sessionStorage에 캡처되며,
  // API 호출 시 X-User-Id 헤더로 전달된다. 여기서는 userId 쿼리 파라미터를 제거하지 않는다.
