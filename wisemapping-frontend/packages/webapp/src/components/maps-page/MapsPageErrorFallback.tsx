...
const MapsPageErrorFallback = (): ReactElement => {
  ...
  const [tryingErpSession, setTryingErpSession] = useState(false);
  ...
  // Try ERP session-based auto-login when auth error occurs
  useEffect(() => {
    if (isAuthError && !tryingErpSession) {
      setTryingErpSession(true);
      const tryErpSessionLogin = async (): Promise<void> => {
        try {
          await client.loginFromSession();
          // If successful, reload the page
          window.location.reload();
        } catch {
          // ERP session login failed, show login UI
  ...
  }, [intl, isAuthError, client, tryingErpSession]);
