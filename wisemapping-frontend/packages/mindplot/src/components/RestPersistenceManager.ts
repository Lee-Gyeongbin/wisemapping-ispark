...
class RESTPersistenceManager extends PersistenceManager {
  ...
  private _buildHttpHeader(contentType: string, accept?: string) {
    ...
    // ERP iframe integration: userId 기반(무상태) 인증을 위해 항상 전달
    try {
      const erpUserId = window?.sessionStorage?.getItem('erp.userId');
      if (erpUserId && erpUserId.trim().length > 0) {
        // eslint-disable-next-line dot-notation
        headers['X-User-Id'] = erpUserId.trim();
      }
    } catch {
      // ignore
    }
