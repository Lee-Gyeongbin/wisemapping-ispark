...
public class MindmapManagerImpl
    ...
    public void updateCollaboration(@NotNull Collaboration collaboration) {
        ...
        // Explicitly merge collaboration properties to ensure starred status is
        // persisted
    ...
    @Override
    @NotNull
    public Collaboration findOrCreateCollaboration(@NotNull Mindmap mindmap, @NotNull Collaborator collaborator,
            @NotNull CollaborationRole role) {
        // Use JPA Criteria API for type-safe query
        final CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        final CriteriaQuery<Collaboration> cq = cb.createQuery(Collaboration.class);
        final Root<Collaboration> root = cq.from(Collaboration.class);
    ...
    public List<Mindmap> findMindmapByUser(@NotNull Account user) {
        ...
        // Load creator and labels (plus label creator) in a single query to avoid N+1
        // access later
    ...
    public List<Collaboration> findCollaboration(final int collaboratorId) {
        ...
        final TypedQuery<Collaboration> query = entityManager.createNamedQuery("Collaboration.findByCollaboratorId",
                Collaboration.class);
    ...
    public Mindmap getMindmapById(int id) {
        // Use Criteria API with JOIN FETCH to explicitly load Account (not Collaborator
        // proxy)
        ...
        root.fetch("creator", JoinType.LEFT);
        // Eagerly fetch collaborations, their properties, and collaborators to avoid
        // lazy loading issues
        final jakarta.persistence.criteria.Fetch<Mindmap, Collaboration> collaborationsFetch = root
                .fetch("collaborations", JoinType.LEFT);
        collaborationsFetch.fetch("collaborationProperties", JoinType.LEFT);
        collaborationsFetch.fetch("collaborator", JoinType.LEFT);
    ...
    public Mindmap getMindmapByTitle(final String title, final Account user) {
        // Use Criteria API with JOIN FETCH to explicitly load Account creator (not
        // Collaborator proxy)
        // This avoids proxy narrowing warnings for JOINED inheritance
        final CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        final CriteriaQuery<Mindmap> cq = cb.createQuery(Mindmap.class);
    ...
    public void updateMindmap(@NotNull Mindmap mindMap, boolean saveHistory) {
        assert mindMap != null : "Save Mindmap: Mindmap is required!";
        // Handle spam info separately using native SQL to prevent duplicate key
        // violations
        MindmapSpamInfo spamInfo = mindMap.getSpamInfo();
        if (spamInfo != null) {
            updateMindmapSpamInfo(spamInfo);
     ...
     * This ensures that cached Collaborator entities with stale collaboration
     * collections
    ...
    private void evictCollaboratorCache(java.util.Set<Integer> collaboratorIds) {
        ...
        try {
            org.hibernate.SessionFactory sessionFactory = entityManagerFactory
                    .unwrap(org.hibernate.SessionFactory.class);
    ...
    public void removeMindmap(@NotNull final Mindmap mindmap) {
        ...
        if (!managedMindmap.getCollaborations().isEmpty()) {
            final java.util.Set<Collaboration> collaborations = new java.util.HashSet<>(
                    managedMindmap.getCollaborations());
    ...
    @Override
    public List<SpamUserResult> findUsersWithSpamMindapsCursor(int spamThreshold, int monthsBack, Integer lastUserId,
            int limit) {
        final TypedQuery<Object[]> query = entityManager.createQuery(
                "SELECT m.creator, COUNT(m.id) as spamCount " +
                        "FROM com.wisemapping.model.Mindmap m " +
    ...
    @Override
    public List<SpamRatioUserResult> findUsersWithHighSpamRatio(int minSpamCount, double spamRatioThreshold,
            int monthsBack, int offset, int limit) {
        final TypedQuery<Object[]> query = entityManager.createQuery(
                "SELECT m.creator, " +
    ...
    @Override
    public List<SpamUserResult> findUsersWithMinimumMapsAndSpam(int minTotalMaps, int minSpamCount, int monthsBack,
    ...
    @Override
    public List<SpamRatioUserResult> findUsersWithHighPublicSpamRatio(double spamRatioThreshold, int monthsBack,
            int offset, int limit) {
        // Optimized: Select Account ID instead of full Account entity to avoid eager
        // loading of large fields
        ...
        // Convert Object[] results to SpamRatioUserResult objects
        // Load Account entities separately to avoid eager loading of unnecessary
        // relationships
    ...
    public long countUsersWithHighPublicSpamRatio(double spamRatioThreshold, int monthsBack) {
        ...
        // Uses outer COUNT over subquery to ensure we always get a result (0 if no
        // matches),
    ...
    @Override
    public List<Mindmap> findPublicMindmapsNeedingSpamDetection(Calendar cutoffDate, int currentVersion, int offset,
            int limit) {
        final TypedQuery<Mindmap> query = entityManager.createQuery(
    ...
    @Override
    public List<SpamUserResult> findUsersWithPublicSpamMapsByType(String[] spamTypeCodes, int monthsBack, int offset,
            int limit) {
     ...
     * - Only cleans up history for mindmaps that haven't been modified in the last
     * 1 week
     * - Cleans history with excess entries or old entries for mindmaps between 1
     * week and 2 years old
     *
    ...
    /**
     * Remove excess history entries for a mindmap, keeping only the most recent
     * ones
     * 
    ...
    public List<Mindmap> searchMindmaps(String search, Boolean filterPublic, Boolean filterLocked, Boolean filterSpam,
        ...
        if (search != null && !search.trim().isEmpty()) {
            if (isIdSearch) {
                ...
                queryString.append(
                        " AND (LOWER(m.title) LIKE LOWER(:search) OR LOWER(m.description) LIKE LOWER(:search))");
            }
        }
        if (filterPublic != null) {
            queryString.append(" AND m.isPublic = :filterPublic");
    ...
    public List<Collaborator> searchCollaborators(String searchTerm, int offset, int limit) {
        final CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        final CriteriaQuery<Collaborator> cq = cb.createQuery(Collaborator.class);
        final Root<Collaborator> root = cq.from(Collaborator.class);
        
        // 이메일로 검색 (COLLABORATOR 테이블의 모든 레코드 검색)
        final String searchPattern = "%" + searchTerm.toLowerCase() + "%";
        cq.select(root)
                .where(cb.like(cb.lower(root.get("email")), searchPattern))
