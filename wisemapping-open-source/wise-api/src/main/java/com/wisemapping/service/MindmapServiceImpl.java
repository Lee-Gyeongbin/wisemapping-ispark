...
public class MindmapServiceImpl
    ...
    @Override
    @Nullable
    // NOTE: 동시 편집/락 동작 테스트를 위해 잠시 권한 체크 비활성화.
    // 원래 코드:
    ...
    public void removeCollaboration(@NotNull Mindmap mindmap, @NotNull Collaboration collaboration)
        ...
        if (creator.identityEquality(collaboration.getCollaborator())) {
            throw new CollaborationException("User is the creator and must have ownership permissions.Creator Email:"
                    + mindMap.getCreator().getEmail() + ",Collaborator:" + collaboration.getCollaborator().getEmail());
        }
        ...
        // When you delete an object from hibernate you have to delete it from *all*
        // collections it exists in...
    ...
    public void removeMindmap(@NotNull Mindmap mindmap, @NotNull Account user) throws WiseMappingException {
        if (mindmap.getCreator().identityEquality(user)) {
            mindmapManager.removeHistoryByMindmapId(mindmap.getId());
    ...
    public void addMindmap(@NotNull Mindmap mindmap, @NotNull Account user) {
        ...
        Account dbUser = userService.getUserBy(user.getId());
        if (dbUser == null && user.getEmail() != null && !user.getEmail().isBlank()) {
            dbUser = userService.findOrCreateAccountByEmail(user.getEmail().trim().toLowerCase());
        }
        if (dbUser == null) {
            throw new IllegalArgumentException("User account not found and could not be created for: " + user.getEmail());
        }
    ...
    public void addCollaboration(@NotNull Mindmap mindmap, @NotNull String email, @NotNull CollaborationRole role,
        ...
        final Collaboration existingCollaboration = mindmapManager.findCollaboration(mindmap.getId(),
                collaborator.getId());
    ...
    private void validateCollaborationRequest(@NotNull Mindmap mindmap, @NotNull String email,
    ...
    public void updateCollaboration(@NotNull Collaborator collaborator, @NotNull Collaboration collaboration)
            throws WiseMappingException {
