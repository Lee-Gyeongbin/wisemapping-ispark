...
public class UserServiceImpl
    ...
    @Override
    @NotNull
    public Account findOrCreateAccountByEmail(@NotNull String email) {
        Account user = userManager.getUserBy(email);
        if (user != null) {
            return user;
        }
        ...
        try {
            ...
            logger.warn("Failed to create account for {}: {}", email, e.getMessage());
            user = userManager.getUserBy(email);
            if (user != null) {
                return user;
            }
            throw new RuntimeException("Could not find or create account for " + email, e);
        }
    }
