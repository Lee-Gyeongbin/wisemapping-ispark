...
public class UserDetails implements org.springframework.security.core.userdetails.UserDetails {
     ...
     * When account does not exist (e.g. external/iframe user), auto-creates via findOrCreateAccountByEmail.
     */
    public Account getUser() {
        Account freshUser = getFreshAccount();
        if (freshUser == null && userService != null && getUsername() != null && !getUsername().isBlank()) {
            freshUser = userService.findOrCreateAccountByEmail(getUsername().trim().toLowerCase());
        }
        if (freshUser == null) {
