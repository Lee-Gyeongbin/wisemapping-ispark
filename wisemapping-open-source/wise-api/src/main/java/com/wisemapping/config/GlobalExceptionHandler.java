...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(com.wisemapping.exceptions.LockException.class)
    @ResponseStatus(HttpStatus.CONFLICT)
    @ResponseBody
    public RestErrors handleLockException(@NotNull com.wisemapping.exceptions.LockException ex) {
        logger.warn("LockException: mindmap lock error - {}", ex.getMessage());
        return new RestErrors(ex.getMessage(), Severity.WARNING);
    }
    ...
    public RestErrors handleBadCredentialsException(@NotNull BadCredentialsException ex) {
        logger.warn("Login failed (401): {}", ex.getMessage());
        final Locale locale = LocaleContextHolder.getLocale();
        String message = messageSource != null ? 
