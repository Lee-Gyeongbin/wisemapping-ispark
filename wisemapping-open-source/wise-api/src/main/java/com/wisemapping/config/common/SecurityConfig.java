...
public class SecurityConfig {
    ...
    @Value("${app.security.sessionUserIdBypassEnabled:false}")
    private boolean sessionUserIdBypassEnabled;

    @Value("${app.security.sessionUserIdAttribute:userId}")
    private String sessionUserIdAttribute;

    @Value("${app.auth.com-userinfo.table:com_userinfo}")
    private String comUserInfoTable;
    @Value("${app.auth.com-userinfo.user-id-column:USER_ID}")
    private String comUserInfoUserIdColumn;
    @Value("${app.auth.com-userinfo.password-column:PASSWD}")
    private String comUserInfoPasswordColumn;
    /** StandardPasswordEncoder용 secret (기존 시스템과 동일하게 맞출 때 설정) */
    @Value("${app.auth.standard-password-encoder.secret:}")
    private String standardPasswordEncoderSecret;
    ...
    /**
     * StandardPasswordEncoder for com_userinfo password matching (legacy).
     * 기존 passwordEncoder.matches(password, user.getPasswd()) 와 동일한 방식으로 매치.
     * secret 사용 시 application.yml에 app.auth.standard-password-encoder.secret 설정.
     */
    @Bean
    @SuppressWarnings("deprecation")
    public StandardPasswordEncoder standardPasswordEncoder() {
        return (standardPasswordEncoderSecret != null && !standardPasswordEncoderSecret.isBlank())
                ? new StandardPasswordEncoder(standardPasswordEncoderSecret)
    ...
    public AuthenticationProvider dbAuthenticationProvider() {
        ...
        provider.setTransactionTemplate(new TransactionTemplate(transactionManager));
        provider.setSessionUserIdBypassEnabled(sessionUserIdBypassEnabled);
        provider.setSessionUserIdAttribute(sessionUserIdAttribute);
        provider.setComUserInfoTable(comUserInfoTable);
        provider.setComUserInfoUserIdColumn(comUserInfoUserIdColumn);
        provider.setComUserInfoPasswordColumn(comUserInfoPasswordColumn);
        provider.setStandardPasswordEncoder(standardPasswordEncoder());

        logger.info("DATABASE authentication provider initialized (ID/PW from {} only)", comUserInfoTable);
        return provider;
    }
